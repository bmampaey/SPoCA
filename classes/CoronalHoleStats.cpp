...
CoronalHoleStats::CoronalHoleStats(const time_t& observationTime)
...
CoronalHoleStats::CoronalHoleStats(const time_t& observationTime, const unsigned id, const unsigned long color)
...
Coordinate CoronalHoleStats::Barycenter() const
{
...
Real CoronalHoleStats::CenterxError() const
{
	if (numberPixels > 0)
	{
...
Real CoronalHoleStats::CenteryError() const
{
	if (numberPixels > 0)
	{
...
Real CoronalHoleStats::MinIntensity() const
{
...
Real CoronalHoleStats::MaxIntensity() const
{
...
Real CoronalHoleStats::Mean() const
{
	if(numberPixels == 0)
		return 0;
...
void CoronalHoleStats::computeMoments()
{
	Real mean = Mean();
...
Real CoronalHoleStats::Variance() const
{
	if(numberPixels == 0)
		return 0;
	if(isnan(m2))
	{
		const_cast<CoronalHoleStats*>(this)->computeMoments();
	}
...
Real CoronalHoleStats::Skewness() const
{
	if(numberPixels == 0)
		return 0;
...
Real CoronalHoleStats::Kurtosis() const
{
	if(numberPixels == 0)
		return 0;
...
Real CoronalHoleStats::TotalIntensity() const
{
...
Real CoronalHoleStats::Area_Raw() const
{
	return area_Raw;
}		

Real CoronalHoleStats::Area_RawUncert() const
{
	return area_RawUncert;
}

Real CoronalHoleStats::Area_AtDiskCenter() const
{
	return area_AtDiskCenter;
}		

Real CoronalHoleStats::Area_AtDiskCenterUncert() const
{
	return area_AtDiskCenterUncert;
}


string CoronalHoleStats::toString(const string& separator, bool header) const
{
...
vector<CoronalHoleStats*> getCoronalHoleStats(const ColorMap* colorizedComponentsMap, const EUVImage* image)
{
	map<unsigned,CoronalHoleStats*> regions_table;
	...
	for (unsigned y = 0; y < colorizedComponentsMap->Yaxes(); ++y)
	{
		for (unsigned x = 0; x < colorizedComponentsMap->Xaxes(); ++x)
		{
			if(colorizedComponentsMap->pixel(x,y) != colorizedComponentsMap->nullvalue())
			{
				...
				{
					regions_table[color] = new CoronalHoleStats(colorizedComponentsMap->ObservationTime(),id, color);
					++id;
				}
	...
	vector<CoronalHoleStats*> regions;
	regions.reserve(regions_table.size());
	for(map<unsigned,CoronalHoleStats*>::const_iterator r = regions_table.begin(); r != regions_table.end(); ++r)
