...
IF (debug GT 0) THEN BEGIN
	...
	FOR e = 1, N_ELEMENTS(saved_events) - 1 DO BEGIN
		...
		IF (debug GT 1) THEN PRINT, saved_events[e].info
	ENDFOR
ENDIF
...
IF N_ELEMENTS(minLifeTime) EQ 0 THEN minLifeTime = 5 * 24 * 3600.0D
IF N_ELEMENTS(minDeathTime) EQ 0 THEN minDeathTime = 21600.0D
...
IF ~ FILE_TEST( spoca_bin, /EXECUTABLE)  THEN BEGIN
	...
	RETURN
ENDIF
IF N_ELEMENTS(spocaArgsPreprocessing) EQ 0 THEN spocaArgsPreprocessing = 'ALC,DivExpTime,TakeSqrt'  
...
; We read the table of Coronal Hole
region_table = MRDFITS(last_map , 'CoronalHoleStats', region_table_header, extnum=extnum, status=status) 
...
; We read the table of CoronalHole Chaincode
chaincode_table = MRDFITS(last_map , 'CoronalHoleChaincode', chaincode_table_header, extnum=extnum, status=status) 

IF size(chaincode_table, /tn) NE "STRUCT" THEN BEGIN

	number_chaincode_points = 0
	IF (debug GT 0) THEN BEGIN
...
ENDIF ELSE BEGIN
	...
	number_chaincode_points = N_ELEMENTS(chaincode_table) ; This is the number of points in each chain code
...
FRM_ParamSet = 'image195 : calibrated image 193/195 A' $
		...
		+ '; minLifeTime='              + STRING(minLifeTime, FORMAT='(I0)') $
		+ '; minDeathTime='             + STRING(minDeathTime, FORMAT='(I0)') 
...
FOR k = 0, number_events - 1 DO BEGIN 
	...
	cartesian_x = FLOAT([region_table[k].XCENTER,region_table[k].XBOXMIN, region_table[k].XBOXMAX])
	cartesian_y = FLOAT([region_table[k].YCENTER,region_table[k].YBOXMIN, region_table[k].YBOXMAX])
	...
	IF number_chaincode_points GT 0 THEN BEGIN
		x_column = where(chaincode_columns EQ STRING(k, FORMAT='("X",I07)'))
		y_column = where(chaincode_columns EQ STRING(k, FORMAT='("Y",I07)'))
	...
	IF exists EQ 0 THEN BEGIN
		...
		ENDIF
		meta_events = [meta_events, {meta_event, color:color, first_seen:current_observation_date, last_seen:current_observation_date, last_ivorn:event.required.kb_archivid}]
	ENDIF ELSE BEGIN
		IF (debug GT 0) THEN BEGIN
		...
		IF exists GT 0 THEN BEGIN
			...
			IF (debug GT 0) THEN BEGIN
				...
				FOR e = 0, N_ELEMENTS(ripe_events) - 1 DO BEGIN
					...
					IF (debug GT 1) THEN PRINT, saved_events[ripe_events[e]].info
				ENDFOR
			ENDIF
...
ENDIF ELSE IF (debug GT 0) THEN BEGIN
		...
		IF exists GT 0 THEN BEGIN
			IF (debug GT 0) THEN BEGIN
				...
				FOR e = 0, N_ELEMENTS(dead_events) - 1 DO BEGIN
					...
					IF (debug GT 1) THEN PRINT, saved_events[dead_events[e]].info
				ENDFOR
			ENDIF
...
; we save the CHmap corresponding to the events we write
IF (debug GT 0 AND STRLEN(saveDirectory) NE 0) THEN BEGIN
	FILE_COPY, CHmaps[N_ELEMENTS(CHmaps) - 1], saveDirectory, /NOEXPAND_PATH, /OVERWRITE, /REQUIRE_DIRECTORY, /VERBOSE 
ENDIF
...
; We save the centers file for debugging
IF (debug GT 0 AND STRLEN(saveDirectory) NE 0) THEN BEGIN
