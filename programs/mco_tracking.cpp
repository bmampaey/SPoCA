...
int main(int argc, const char **argv)
{
	...
	// We create a edge between 2 nodes if the overlap
	...
	{	
		for (unsigned s1 = 0; d + s1 < images.size(); ++s1)
		{
			...
			{
				// We check that the region has not been tracked already (== has no sons)
				if(tracking_graph.get_node(regions[s1][r1]).adjlist().size() != 0)
					continue;
	...
	ouputGraph(tracking_graph, regions, "ar_graph_original");
	#endif

	// We build the TargetSet graph
	TrackGraph tracks;
	vector<TargetSet*> fakeTs;
	vector< vector<TargetSet*> > regions2targetsets(regions.size());
	for (unsigned s = 0; s < regions.size(); ++s)
	{
		regions2targetsets[s].resize(regions[s].size(), NULL);
		for (unsigned r = 0; r < regions[s].size(); ++r)
		{
			RegionGraph::node r = tracking_graph.get_node(regions[s][r]);
			
		
		// Case merge
		if(r.iadjlist().size() > 1)
		{
			regions2targetsets[s][r] = new TargetSet(regions[s][r]);
			// Case merg - split
			if(itn->adjlist().size() > 1)
			{
				// We insert fake nodes when there is merge-split
				
			}
		}
		// Case split
		else if(itn->adjlist().size() > 1)
		{
	...
	for (unsigned s = 0; s < regions.size(); ++s)
		for (unsigned r = 0; r < regions[s].size(); ++r)
		{
			...
			// We insert fake nodes when there is merge-split
			if(r.adjlist().size() > 1 && r.iadjlist().size() > 1)
			{
				Region* fakeRegion = new Region(regions[s][r]);
				fakeregions.push_back(fakeRegion);
				Vertex * Vi = new Vertex;
				Vi.tracks.push_back(StateVector(regions[s][r]));
				G1.insert_vertex(Vi);
			}
			if(r.iadjlist().size() == 1
		...
		
		{
			...
			Vertex * Vi = new Vertex
			tracking_graph.insert_vertex(fakeRegion);
			RegionGraph::node fakeNode = tracking_graph.get_node(fakeRegion);
			// NOK 
			fakeNode->adjlist() = itn->adjlist();
			itn->adjlist().clear();
			tracking_graph.insert_edge(itn->value()->NumberPixels(), itn->value(), fakeRegion);
		}
					Region* fakeRegion = new Region(itn->value());
			fakeregions.push_back(fakeRegion);
			
	}

	// We build the inference graph
	InferenceGraph G1;
