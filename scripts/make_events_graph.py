...
if __name__ == "__main__":
	...
	for filename in filenames:
		...
		# We create the node
		event_node = pydot.Node(name=event.uri, label=label, color=color, tooltip=tooltip, URL=filename, shape='box')
	
		# We add the node to the list of subgraphs
		if event.time not in nodes_times:
			nodes_times[event.time] = [event_node]
		else:
			nodes_times[event.time].append(event_node)
	
		# We add the node to the list of edges
		events_links[event.uri] = event_links
	
	graph = pydot.Dot('"'+graphname+'"', graph_type='digraph', rankdir='TB', pack=False)
		...
		for src_node, links in events_links.items():
			for dest_node, label in links.items():
				if label == "merges_from":
					graph.add_edge(pydot.Edge(dest_node, src_node, label="merges_into", weight=1))
				elif label == "splits_from":
					graph.add_edge(pydot.Edge(dest_node, src_node, label="splits_into", weight=1))
				else:
					graph.add_edge(pydot.Edge(dest_node, src_node, label=label, weight=10))
	else:
		for src_node, links in events_links.items():
			for dest_node, label in links.items():
				if label == "follows":
