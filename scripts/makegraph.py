...
def get_coordinates(element):
	return (str(element.getElementsByTagName('C1')[0].firstChild.data), str(element.getElementsByTagName('C2')[0].firstChild.data))
...
def parse_event(filename):
	...
	for p in dom.getElementsByTagName('Param'):
		event_info[p.getAttribute("name").lower()] = p.getAttribute("value")
	event_info["time"] = get_date(dom.getElementsByTagName('TimeInstant')[0])
	event_info["start_time"] = get_date(dom.getElementsByTagName('StartTime')[0])
	event_info["stop_time"] = get_date(dom.getElementsByTagName('StopTime')[0])
	event_info["center"] = get_coordinates(dom.getElementsByTagName('Value2')[0])
	event_info["center_error"] = get_coordinates(dom.getElementsByTagName('Error2')[0])
	event_info["box_center"] = get_coordinates(dom.getElementsByTagName('Center')[0])
	event_info["box_size"] = get_coordinates(dom.getElementsByTagName('Size')[0])
	event_info['type'] = dom.getElementsByTagName('Concept')[0].firstChild.data
	event_info['uri'] = get_uri(dom.getElementsByTagName('AuthorIVORN')[0].firstChild.data)
	event_info['color'] = int(re.search(r"(\d+)$", event_info['frm_specificid']).group(1))
...
if __name__ == "__main__":
	...
	for filename in filenames:
		...
		event_info, event_links = parse_event(filename)
	
		# We create the label
		if not keywords:
			label = event_info['uri']
		else:
			label = ""
			for k in keywords:
				if k.lower() in event_info:
					label += k + ": " + str(event_info[k.lower()]) + "\l"
		# We create the tooltip
		tooltip = ""
		for k in event_info.keys():
			tooltip += k + ": " + str(event_info[k.lower()]) + "&#010; "
	
		# We assign the right color
		color = gradient[event_info['color']%len(gradient)]
	
		# We create the node
		event_node = pydot.Node(name=event_info['uri'], label=label, color=color, tooltip=tooltip, URL=filename, shape='box')
	
		# We add the node to the list of subgraphs
		if event_info['time'] not in nodes_times:
			nodes_times[event_info['time']] = [event_node]
		else:
			nodes_times[event_info['time']].append(event_node)
	
		# We add the node to the list of edges
		events_links[event_info['uri']] = event_links
	
	graph = pydot.Dot('"'+graphname+'"', graph_type='digraph', rankdir='LR')
	
	# We create the subgraphes and add them to the main graph
	for time, nodes in nodes_times.items():
	...
	if inversed:
		for src_node, links in events_links.items():
			for dest_node, label in links.items():
				if label == "merges_from":
					graph.add_edge(pydot.Edge(dest_node, src_node, label="merges_into"))
				elif label == "splits_from":
					graph.add_edge(pydot.Edge(dest_node, src_node, label="splits_into"))
				else:
					graph.add_edge(pydot.Edge(dest_node, src_node, label=label))
	else:
		for src_node, links in events_links.items():
			for dest_node, label in links.items():
				graph.add_edge(pydot.Edge(src_node, dest_node , label=label))
	...
	try:
		with open(graphname+'.html', 'w+') as f:
			f.write('<IMG SRC="'+graphname+'.png" USEMAP="#'+graphname+'" />\n')
			f.write(cmapx)
	except IOError, why:
