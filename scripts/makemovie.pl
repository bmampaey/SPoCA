...
my $processing = ' -alpha set -fill none -draw "color 0,0 replace" -crop 1800x1500+124+360 +repage -resize 33% ';
...
my $datarepository = 'data';
my @wavelengths = ("171", "195");
my @algos = ("FCM.P5", "HistogramFCM.P5");
my $fitssuffix = 'fts';
...
for my $algo (@algos)
{
	...
	my $resultsrepository = $resultsrepositoryprefix.'.'.$algo;
	...
	{	
		...
		my $annotate = $convert . $name . $annotation . $algo.' '.$name;
		if (! $all)
		{
			print "\nAbout to do : \n\t$colorize\n\t$hollow\n\t$annotate\nIs it ok ? [yes/no/all/none] :";
			$answer = <STDIN>;
	...
	for my $wavelength (@wavelengths)
	{
		...
		my $movierepository = $movierepositoryprefix.'.'.$algo.'.'.$wavelength;
