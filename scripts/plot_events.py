...
if __name__ == "__main__":
	...
	threads = list()
	delta = int(len(filenames)/args.threading) + 1
	start = 0
	while len(threads) < args.threading:
		threads.append(threading.Thread(group=None, target=parse_events, name='parse_events_'+str(len(threads)), args=(filenames[start:start+delta], events_queue), kwargs={}))
		threads[-1].start()
		start += delta
