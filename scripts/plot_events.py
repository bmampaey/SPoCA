...
def plot_attributes(filename, meta_events, attributes, time_marker=None, time_limits=None, size=None, legend=False):
	...
	for attribute in attributes:
		...
		if time_marker:
			axes[attribute].axvline(x=time_marker, linewidth=4, color='black', alpha=0.3)
...
if __name__ == "__main__":
	...
	try :
		...
		print "Could not write colors to file colors.txt"
	
	
	if args.attribute:
		# We plot everything
		plot_attributes(args.output, meta_events, args.attribute, legend=args.legend, size=size)
		...
		if args.multi != None:
			...
			for t in time_ticks:
				...
				plot_attributes(pretty_date(t)+'.'+args.output, meta_events, args.attribute, time_marker=t, time_limits=(tmin, tmax), legend=args.legend, size=size)
