...
def make_segmentation_jobs(filenames_queue, output_queue):
	...
	filename = filenames_queue.get()
	# None is provided when the queue is empty
	while filename != None:
		
		log.debug("Received filename " + str(filename))
		
		date_obs, centers, image1, image2 = get_keywords(filename, ['T_OBS', 'CCENTER', 'IMAGE001', 'IMAGE002'])
		
		reference_string = pretty_date(dateutil.parser.parse(date_obs))
		...
		if force or not os.path.exists(filename):
			...
			fitsfiles = [image1] if not image2 else [image1, image2]
			center_filename = os.path.join(segmentation['repo'], reference_string + ".centers.txt")
			try:
				with open(center_filename, 'w+') as f:
					f.write(centers)
			except IOError, why:
				log.critical("Could not write centers to file "+ str(center_filename))
				continue
			
			arguments = {'args':'-O ' + segmentation['repo'] + ' ' + segmentation['args'] + ' -B ' + center_filename + ' ', 'fitsfiles':fitsfiles}
