...
def rewrite_map(input_filepath, image_hdu_name_or_index, output_filepath, longlived_regions_colors, background_color = 0):
	...
	with fits.open(input_filepath) as hdulist:
		...
		erase_colors = set(numpy.unique(image)) - set(longlived_regions_colors) - set([background_color])
...
if __name__ == '__main__':
	...
	# Get the arguments
	parser = ArgumentParser(description='Rewrite SPoCA map FITS files but only keep regions that have a minimum lifespan')
	...
	for filepath in args.filepaths:
		try:
			...
			logging.critical('Could not rewrite SPoCA map %s: %s', filepath, why)
			raise
