...
def make_get_stats_jobs(files_queue, output_queue, force=False):
	...
	while files:
		...
		job = spoca_job.get_STAFF_stats(job_name, CHmapfile=files[0], ARmapfile=files[1], fitsfiles=files[2:], force = force)
		if job.job:
			logging.info("Running get_stats job %s for CHMap %s and ARMap %s on files %s", job_name, files[0], files[1], str(files[2:]))
			output_queue.put(job.job)
		else:
			logging.info("Skipping get_stats job %s for CHMap %s and ARMap %s on files %s", job_name, files[0], files[1], str(files[2:]))
...
if __name__ == "__main__":
	...
	# Get the arguments
	parser = argparse.ArgumentParser(description='Run get_stats in parralel on many maps.')
	...
	parser.add_argument('--output_directory', '-o', default='images', help='Set the output directory for the stats')
	...
	parser.add_argument('--ARMaps', '-A', nargs='+', help='The paths of the ARMaps')
	parser.add_argument('--CHMaps', '-C', nargs='+', help='The paths of the CHMaps')
	...
	# get_stats config
	if args.get_stats_config == None or not os.path.exists(args.get_stats_config):
		log.critical("Config file %s does not exists", args.get_stats_config)
		sys.exit(2)
	else:
		spoca_job.get_stats.set_parameters(args.get_stats_config, output_directory, args.extra)
	
	ok, reason = spoca_job.get_stats.test_parameters()
	if ok:
		log.info("get_stats parameters seem ok")
		log.debug(reason)
	else:
		log.warn("get_stats parameters could be wrong")
		log.warn(reason)	
	
	
	# ARMaps
	if not args.ARMaps:
		log.critical("No ARMaps specified, exiting.")
		sys.exit(2)
	
	# We glob the ARMaps
	ARMaplist = glob_files(args.ARMaps)
	
	if len(ARMaplist) == 0:
		log.critical("No ARMaps found to process, exiting.")
		sys.exit(2)
	
	# CHMaps
	if not args.CHMaps:
		log.critical("No CHMaps specified, exiting.")
		sys.exit(2)
	
	# We glob the CHMaps
	CHMaplist = glob_files(args.CHMaps)
	
	if len(CHMaplist) == 0:
		log.critical("No CHMaps found to process, exiting.")
		sys.exit(2)
	
	if len(CHMaplist) != len(ARMaplist):
		log.warning("The number of CHMaps is different than the number of ARMaps")
	
	number_files = min(len(CHMaplist), len(ARMaplist))
	
	# Files
	filelists = list()
	if not args.files:
		log.info("No fits files specified, using IMAGE001 in CHMaps.")
	else:
		# We glob the files
	...
	filelists.insert(0, ARMaplist)
	filelists.insert(0, CHMaplist)
	if(args.align_files):
		max_delta = None
		if args.max_delta:
